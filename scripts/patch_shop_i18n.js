...
let matchCount = 0;
// We know that `shopBackToTeam` is followed by a quote, maybe a comma, and a newline.
content = content.replace(/(shopBackToTeam: [^,\n]+),?/g, (match, p1, offset, string) => {
  // Determine memory context by searching backwards for lang keys:  " ko:", " en:", etc.
  const beforeStr = string.substring(0, offset);
  const koIx = beforeStr.lastIndexOf('ko: {');
  const enIx = beforeStr.lastIndexOf('en: {');
  const msIx = beforeStr.lastIndexOf('ms: {');
  const idIx = beforeStr.lastIndexOf('id: {');
  const viIx = beforeStr.lastIndexOf('vi: {');
  const thIx = beforeStr.lastIndexOf('th: {');
  // Find which one is visually closest (largest index)
  const maxIx = Math.max(koIx, enIx, msIx, idIx, viIx, thIx);
  let isKo = maxIx === koIx;
  matchCount++;
  console.log('Replaced block at language index ' + maxIx);
  return p1 + ",\\n" + (isKo ? t.ko : t.en);
});

fs.writeFileSync(dictPath, content);
console.log('Successfully patched dictionaries.ts ' + matchCount + ' times!');
