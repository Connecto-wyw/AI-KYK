...
// Reset back to cleanest baseline just in case
let content = fs.readFileSync(dictPath, 'utf8');

// We are going to just replace what we did last time. Wait, if I've already patched ko, en, ms, id, vi, I should revert the file to git's pristine state FIRST directly inside the script.
const execSync = require('child_process').execSync;
execSync('git checkout src/lib/i18n/dictionaries.ts', { stdio: 'ignore' });

content = fs.readFileSync(dictPath, 'utf8');
...
langs.forEach(lang => {
  ...
  // Find the VERY LAST non-empty line of the block to inject a comma if it lacks one, 
  // then inject our text right before the block closure.
  // We can just replace the trailing `\n  },` of the block with `,\n` + appendText + `\n  },`.
  // First extract the last non-empty char of the block.
  const trimmed = block.trimEnd();
  const hasComma = trimmed.endsWith(',');
  
  // Replace the exact exact string representation from blockEnd onwards exclusively within this block
  const newBlock = trimmed + (hasComma ? '\\n' : ',\\n') + appendText + '\\n';
